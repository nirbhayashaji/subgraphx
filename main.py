...
def main():
    ...
    INPUT_FILE = BASE_DIR / "data" / "raw_network_transactions.csv"
    ...
    target_nodes = ["513902872", "509903851", "516096826"]

    analysis_output_path = RESULTS_DIR / "structural_features.csv"
    domain_output_path = RESULTS_DIR / "domain_features.csv"
    master_features_path = RESULTS_DIR / "master_node_features.csv"
    global_summary_path = RESULTS_DIR / "global_graph_summary.csv"

    use_fast_track = 'no' # Default state

    # --- 2. SUPER FAST-TRACK CHECK ---
    if GEPHI_FILE.exists() and TRANSFER_FILE.exists():
        logger.info(f"Found pre-calculated structural metrics: {GEPHI_FILE.name}")
        logger.info(f"Found pre-calculated domain metrics: {TRANSFER_FILE.name}")
        ...
        if use_fast_track == 'yes':
            ...
            df_gephi.rename(columns={gephi_id_col: 'Node_ID'}, inplace=True)
            df_transfer.rename(columns={transfer_id_col: 'Node_ID'}, inplace=True)
            
            df_gephi['Node_ID'] = df_gephi['Node_ID'].astype(str)
            df_transfer['Node_ID'] = df_transfer['Node_ID'].astype(str)
            
            logger.info(f"Joining {len(df_gephi.columns)-1} Structural features and {len(df_transfer.columns)-1} Domain features...")
            
            df_master = pd.merge(df_gephi, df_transfer, on='Node_ID', how='outer')
            ...
            for col in df_master.columns:
                if col != 'Node_ID' and pd.api.types.is_numeric_dtype(df_master[col]):
            ...
            # Note: sys.exit() removed from here so execution can continue to Step 6!

    # --- 3 & 4 & 5. STANDARD PIPELINE (Only runs if Fast-Track was NOT selected) ---
    if use_fast_track != 'yes':
        recalc = input("\nDo you want to recalculate all features from scratch via NetworkX? (yes/no): ").strip().lower()

        if recalc != 'yes':
            if not analysis_output_path.exists() or not domain_output_path.exists():
        ...
        if recalc == 'yes':
            ...
            G = build_transfer_network(df_graph)
            num_nodes, num_edges = G.number_of_nodes(), G.number_of_edges()
            ...
            create_transfer_features(
                G=G,
                df=df_graph, 
                output_filepath=domain_output_path,
                log_filepath=log_filepath
            )
            logger.info(f"[Timing] Entire create_transfer_features() function took {time.perf_counter() - t_func_trans:.2f}s")

        logger.info("Merging structural and domain features into Master Dataset...")
        try:
            df_net = pd.read_csv(analysis_output_path, low_memory=False)
            df_dom = pd.read_csv(domain_output_path, low_memory=False)
            
            df_master = pd.merge(df_net, df_dom, on='Node_ID', how='outer')
            df_master.fillna(0, inplace=True)
    ...
    # --- 6. RUN ANOMALY DETECTION (This now runs for BOTH paths!) ---
    logger.info("Initializing Phase 2: Anomaly Detection Ensemble...")
    ...
    try:
        ...
        logger.info("Phase 2 complete! Pipeline finished successfully.")
    except Exception as e:
        logger.error(f"Failed to run anomaly ensemble: {e}")
